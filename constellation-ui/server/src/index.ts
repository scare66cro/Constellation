...
// DEBUG — remove when Phase 4 lands. Exercises the broker's install state
// machine FAILED path + install-mode gating without touching any real LP:
// sends FwInstallBegin then FwInstallComponentBegin with role=99 (no fleet
// member matches), captures all FwInstallProgress (140) + FwInstallResult
// (141) envelopes, sends FwInstallAbort, then re-runs the fleet probe to
// confirm the broker came out of install mode cleanly.
const FW_INSTALL_COMPONENT_STATE = [
  'FW_INSTALL_PENDING','FW_INSTALL_BEGIN','FW_INSTALL_ERASING','FW_INSTALL_PUSHING',
  'FW_INSTALL_VERIFYING','FW_INSTALL_ACTIVATING','FW_INSTALL_REBOOTING','FW_INSTALL_CONFIRMED',
  'FW_INSTALL_DONE','FW_INSTALL_SKIPPED','FW_INSTALL_FAILED',
];
const FW_INSTALL_OVERALL_STATE = [
  'FW_INSTALL_OVERALL_IDLE','FW_INSTALL_OVERALL_PROBING','FW_INSTALL_OVERALL_INSTALLING',
  'FW_INSTALL_OVERALL_DONE','FW_INSTALL_OVERALL_FAILED','FW_INSTALL_OVERALL_ABORTED',
];
app.get('/api/_debug/broker-install-fail-test', async (_req, res) => {
  if (!novaBridge) { res.status(400).json({ ok: false, error: 'Nova bridge not available' }); return; }
  const t0 = Date.now();
  const progress: any[] = [];
  let resultEnv: any = null;

  const onMsg = (msgId: number, data: Buffer) => {
    const t_ms = Date.now() - t0;
    if (msgId === MSG_FW_INSTALL_PROGRESS) {
      const f = pbDecodeFields(data);
      const state = pbGetVarint(f, 4);
      const overall = pbGetVarint(f, 9);
      progress.push({
        t_ms, kind: 'FwInstallProgress',
        component_index: pbGetVarint(f, 1),
        component_name:  pbGetString(f, 2),
        target_host:     pbGetString(f, 3),
        state, state_name: FW_INSTALL_COMPONENT_STATE[state] ?? `UNKNOWN(${state})`,
        bytes_written: pbGetVarint(f, 5),
        total_size:    pbGetVarint(f, 6),
        error_code:    pbGetVarint(f, 7),
        error_message: pbGetString(f, 8),
        overall, overall_name: FW_INSTALL_OVERALL_STATE[overall] ?? `UNKNOWN(${overall})`,
      });
    } else if (msgId === MSG_FW_INSTALL_RESULT) {
      const f = pbDecodeFields(data);
      const overall = pbGetVarint(f, 1);
      const components = pbGetAllSubmsg(f, 3).map((cbuf) => {
        const cf = pbDecodeFields(cbuf);
        const cstate = pbGetVarint(cf, 4);
        return {
          component_index:      pbGetVarint(cf, 1),
          component_name:       pbGetString(cf, 2),
          target_host:          pbGetString(cf, 3),
          state: cstate, state_name: FW_INSTALL_COMPONENT_STATE[cstate] ?? `UNKNOWN(${cstate})`,
          error_code:           pbGetVarint(cf, 5),
          error_message:        pbGetString(cf, 6),
          post_install_version: pbGetString(cf, 7),
        };
      });
      resultEnv = {
        t_ms, overall, overall_name: FW_INSTALL_OVERALL_STATE[overall] ?? `UNKNOWN(${overall})`,
        failure_reason: pbGetString(f, 2), components,
      };
    }
  };
  novaBridge.on('message', onMsg);

  // Build FwInstallBegin inner. wire-type 2 (length-delimited) == bytes encoding.
  const zero32 = Buffer.alloc(32, 0x00);
  const beginInner = Buffer.concat([
    encField(1, 2, Buffer.from('bench-fail-test-0.A.193', 'utf8')),  // bundle_version
    encField(2, 2, zero32),                                          // manifest_sha256
    encField(3, 0, encVarint(1)),                                    // component_count=1
    encField(4, 0, encVarint(1024)),                                 // chunk_size=1024
    // field 5 allow_downgrade=false → omit per proto3 zero-suppression
  ]);
  const begin_sent_at_ms = Date.now() - t0;
  novaBridge.sendFwInstallBegin(beginInner);

  // Build FwInstallComponentBegin inner.
  const compBeginInner = Buffer.concat([
    encField(1, 2, Buffer.from('fail-test-component', 'utf8')),  // component_name
    encField(2, 0, encVarint(0)),                                 // component_index=0 FORCE
    encField(3, 0, encVarint(99)),                                // role=99 no-match
    encField(4, 0, encVarint(1024)),                              // total_size
    encField(5, 2, zero32),                                       // image_sha256
    encField(6, 2, Buffer.from('0.A.193+benchtest', 'utf8')),     // expected_version
  ]);
  const componentBegin_sent_at_ms = Date.now() - t0;
  novaBridge.sendFwInstallComponentBegin(compBeginInner);

  // Wait up to 3 s for progress; stop early if result arrives.
  await new Promise<void>((resolve) => {
    const deadline = Date.now() + 3000;
    const tick = () => {
      if (resultEnv || Date.now() >= deadline) return resolve();
      setTimeout(tick, 50);
    };
    tick();
  });

  // Send abort regardless of state.
  const abortInner = encField(1, 2, Buffer.from('bench-fail-test-cleanup', 'utf8'));
  const abort_sent_at_ms = Date.now() - t0;
  novaBridge.sendFwInstallAbort(abortInner);

  // Wait up to 1 s for the post-abort FwInstallResult.
  await new Promise<void>((resolve) => {
    const deadline = Date.now() + 1000;
    const tick = () => {
      if (resultEnv && resultEnv.t_ms >= abort_sent_at_ms) return resolve();
      if (Date.now() >= deadline) return resolve();
      setTimeout(tick, 50);
    };
    tick();
  });
  novaBridge.off('message', onMsg);

  // Post-abort fleet probe — confirms install-mode gating released.
  let post_abort_probe_ok = false;
  let post_abort_probe_members_count = 0;
  try {
    const raw = await novaBridge.sendFleetProbe(5000);
    const fields = pbDecodeFields(raw);
    post_abort_probe_members_count = pbGetAllSubmsg(fields, 1).length;
    post_abort_probe_ok = true;
  } catch { /* leave false */ }

  if (progress.length === 0 && !resultEnv && !post_abort_probe_ok) {
    res.status(504).json({ ok: false, error: 'no progress, no result, post-abort probe failed', elapsed_ms: Date.now() - t0 });
    return;
  }
  res.json({
    ok: true, begin_sent_at_ms, componentBegin_sent_at_ms,
    progress_envelopes: progress, abort_sent_at_ms, result_envelope: resultEnv,
    post_abort_probe_ok, post_abort_probe_members_count, elapsed_ms: Date.now() - t0,
  });
});

// DEBUG — remove when Phase 4 lands. Real TCP-push half-cycle to live STORAGE LP:
// Bank B gets erased (~5 s) but never finalized/activated. Safety gate: pretest
// STORAGE must be on Bank A with bank_a_valid before any envelope leaves.
// Posttest re-probe confirms no unintended commit happened.
function decodeFleetMember(mbuf: Buffer) {
  const f = pbDecodeFields(mbuf);
  return {
    host:           pbGetString(f, 1),
    reachable:      pbGetVarint(f, 2) !== 0,
...
app.get('/api/_debug/broker-install-begin-abort', async (_req, res) => {
  if (!novaBridge) { res.status(400).json({ ok: false, error: 'Nova bridge not available' }); return; }
  const t0 = Date.now();

  // 1. Safety gate — STORAGE must be healthy on Bank A before we touch it.
  let pretest_storage: any = null;
  try {
    const raw = await novaBridge.sendFleetProbe(5000);
    const members = pbGetAllSubmsg(pbDecodeFields(raw), 1).map(decodeFleetMember);
    pretest_storage = members.find((m) => m.host === '10.47.27.2') ?? null;
  } catch (err: any) {
    res.status(412).json({ ok: false, error: `pretest fleet probe failed: ${err?.message ?? err}`, pretest_storage: null });
    return;
  }
  if (!pretest_storage || !pretest_storage.reachable || pretest_storage.current_role !== 1 ||
      pretest_storage.active_bank !== 0 || !pretest_storage.bank_a_valid) {
    res.status(412).json({ ok: false, error: 'pretest STORAGE unhealthy', pretest_storage });
    return;
  }
  ...
  // 2. FwInstallBegin — allow_downgrade=true (field 5) in case LP runs newer.
  const zero32 = Buffer.alloc(32, 0x00);
  const beginInner = Buffer.concat([
    encField(1, 2, Buffer.from('bench-begin-abort-0.A.193', 'utf8')),  // bundle_version
    encField(2, 2, zero32),                                            // manifest_sha256
    encField(3, 0, encVarint(1)),                                      // component_count=1
    encField(4, 0, encVarint(1024)),                                   // chunk_size=1024
    encField(5, 0, encVarint(1)),                                      // allow_downgrade=true
  ]);
  const begin_sent_at_ms = Date.now() - t0;
  novaBridge.sendFwInstallBegin(beginInner);

  // 3. FwInstallComponentBegin role=1 (force-encoded) -> matches STORAGE @ .2.
  const compBeginInner = Buffer.concat([
    encField(1, 2, Buffer.from('storage-begin-abort', 'utf8')),  // component_name
    encField(2, 0, encVarint(0)),                                 // component_index=0 FORCE
    encField(3, 0, encVarint(1)),                                 // role=1 STORAGE FORCE
    encField(4, 0, encVarint(1024)),                              // total_size synthetic
    encField(5, 2, zero32),                                       // image_sha256
    encField(6, 2, Buffer.from('0.A.193+benchtest', 'utf8')),     // expected_version
  ]);
  const componentBegin_sent_at_ms = Date.now() - t0;
  ...
  // 4. Collect progress envelopes for up to 7 s (BEGIN -> ERASING -> PUSHING expected).
  await new Promise<void>((resolve) => {
  ...
  // 5. Abort -> wait up to 2 s for FwInstallResult(overall=ABORTED).
  const abortInner = encField(1, 2, Buffer.from('bench-begin-abort-cleanup', 'utf8'));
  const abort_sent_at_ms = Date.now() - t0;
  novaBridge.sendFwInstallAbort(abortInner);
  await new Promise<void>((resolve) => {
  ...
  // 6. Post-test verification — wait 2 s for cleanup, then re-probe STORAGE.
  await new Promise((r) => setTimeout(r, 2000));
  let posttest_storage: any = null;
  try {
    const raw = await novaBridge.sendFleetProbe(5000);
    const members = pbGetAllSubmsg(pbDecodeFields(raw), 1).map(decodeFleetMember);
    posttest_storage = members.find((m) => m.host === '10.47.27.2') ?? null;
  } catch (err: any) {
    posttest_storage = { error: `posttest probe failed: ${err?.message ?? err}` };
  }

  res.json({
    ok: true, pretest_storage,
    begin_sent_at_ms, componentBegin_sent_at_ms,
    progress_envelopes: progress, abort_sent_at_ms, result_envelope: resultEnv,
    posttest_storage, elapsed_ms: Date.now() - t0,
  });
});
