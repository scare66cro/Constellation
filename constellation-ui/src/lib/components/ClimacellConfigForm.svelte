<script lang="ts">
  ...
  // Shared body of the Climacell Setup page (level2/climacell): efficiency,
  // altitude + units, and the humidifier PIDU values. Rendered from ONE source
  // of truth — the classic touchscreen page AND the "Config" tab of the
  // dashboard climacell modal. Prop contract mirrors FanSpeedForm (L2).
...
<div class="pform pform--{theme}">
  <Card class="mx-2 flex flex-col">
    ...
    <Table class="mb-2 text-size-xl">
      ...
      <Row>
        <Column class="w-5/12 border-r border-gray-400">{ $t('level2.climacell.climacell-humidifier-s') } {#if typeof window !== 'undefined' && window.innerWidth <= 1280}<br/>{/if}{ $t('global.pidu-values') }</Column>
        <Column class="w-7/12">
