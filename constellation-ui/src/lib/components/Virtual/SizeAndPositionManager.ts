...
export default class SizeAndPositionManager {
  ...
  private totalSize: number | undefined;
	private itemSizeGetter: ItemSizeGetter | undefined;

	constructor(options: ManagerOptions) {
		this.itemSize = options.itemSize;
		this.itemCount = options.itemCount;
		this.estimatedItemSize = options?.estimatedItemSize ?? 50;
		if (typeof this.itemSize === 'number') {
			this.itemSizeGetter = () => this.itemSize as number;
		} else if (Array.isArray(this.itemSize)) {
			this.itemSizeGetter = (i) => (this.itemSize as number[])[i];
		} else {
			this.itemSizeGetter = this.itemSize as ItemSizeGetter;
		}
	 ...
	 */
	updateConfig({ itemSize, itemCount, estimatedItemSize }: ManagerOptions) {
		if (itemCount != null) {
	 ...
	 */
	getUpdatedOffsetForIndex({ align = ALIGNMENT.START, containerSize, currentOffset, targetIndex }: any) {
		if (containerSize <= 0) {
			return 0;
		}
	 ...
	 */
	getVisibleRange({ containerSize = 0, offset, overscanCount }: any) {
		const totalSize = this.getTotalSize();
	 ...
	 * @param {number} offset
	 */
	binarySearch({ low, high, offset }: { low: number; high: number; offset: number }) {
		let middle = 0;
		let currentOffset = 0;
