<script lang="ts">
  ...
  // Shared body of the Plenum Setpoints & Alarms page. Rendered in two
  // presentations from a SINGLE source of truth:
  //   1. /level1/plentemp route  → <GellertPage><PlenumSetpointsForm/></GellertPage>
  //   2. /dashboard/plan3d modal → <PlenumSetpointsForm embedded/>
  // Both share the load + writeProto save path below, so a save from the
  // 3D modal hits the same firmware round-trip as the classic page.

  // `wait` / `ready` are bound back out so the GellertPage shell can show
  // its loading + saving chrome. In the modal these are bound to local
  // state and ignored.
  export let wait = false;
  export let ready = false;
  // `embedded` = rendered inside the dashboard modal (no GellertPage). When
  // true we DON'T register $navigationStore.isDirty — that's a swipe-nav
  // page concern; the modal manages its own open/close, and hijacking the
  // dashboard's nav guard would block swiping between remote panels.
  export let embedded = false;

  // `theme` selects the presentation skin. 'light' (default) = the classic
  // touchscreen page look, untouched. 'dark' = compact slate/blue modal
  // skin (see <style> below). The form's content is identical either way;
  // only colour + type scale change. The dashboard modal passes 'dark'.
  export let theme: 'light' | 'dark' = 'light';

  // SaveButton instances, so the modal can flush() on close. On the
  // classic page the SaveButtons' own autoSave-on-navigate handles this;
  // the modal has no route change to trigger it, so it calls flush()
  // explicitly on every close path EXCEPT Cancel (which discards — the
  // component is destroyed on close, so the edit buffer is thrown away).
  let mainSaveBtn: { save: () => Promise<void> } | undefined;
  let rampSaveBtn: { save: () => Promise<void> } | undefined;

  // Imperative save for the modal. Only writes the sub-forms that are
  // actually dirty, reusing each SaveButton's full validation / write path.
  export async function flush(): Promise<void> {
    const tasks: Promise<void>[] = [];
    if (isMainDirty() && mainSaveBtn) tasks.push(mainSaveBtn.save());
    if (isRampDirty() && rampSaveBtn) tasks.push(rampSaveBtn.save());
  ...
  let original: string[] = [];
  // `canEdit` lets the host force edit mode independent of the global
  // access level — the dashboard modal drives it from its own Program
  // toggle, so unlocking there doesn't depend on navigationStore.level
  // propagating into the modal. null = fall back to the level gate (the
  // classic page behaviour).
  ...
  onMount(() => {
    try {
      ...
      // Page-only: the dashboard modal manages its own dismissal and must
      // not hijack the swipe-nav dirty guard.
      if (!embedded) {
...
<Card class="mt-2 flex flex-col container-wide">
    ...
    {#if !onionMode && useRamp}
      <Card class="w-full mx-auto my-2 flex flex-col bg-surface-100">
        ...
        <SaveButton bind:this={rampSaveBtn} {edit} bind:wait={rampWait} data={ramp.rate}
    ...
    <SaveButton bind:this={mainSaveBtn} {edit} bind:wait={wait} data={plensetup} bind:original={original} bind:validation={validation} autoSave
