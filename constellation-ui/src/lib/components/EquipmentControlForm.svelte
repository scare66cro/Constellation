<script lang="ts">
  ...
  // Shared body of the Equipment Control / Status page (level1/equipment):
  // live equipment output/status/switch rows + imperative on/off and refrig
  // diagnostics over /iot/button. Rendered on the classic page AND as the
  // dashboard "Equipment Control" modal (a CONTROL PANEL — imperative, no
  // save/flush; the modal shows a Close-only footer). Page chrome (GellertPage
  // / ScrollableArea / fixed height) is dropped here; the modal body scrolls.
  ...
  // No persisted edit buffer — every action is an immediate /iot/button POST.
  // flush() is a no-op so this satisfies the modal form contract.
  export async function flush(): Promise<void> {}

  type Counts = { leftRows: number, rightRows: number, reserved: number, refrigAdded: number, auxAdded: number };
  ...
  // Refresh rows when SystemStatus updates so the commanded door % tracks
  // PWM_DOORS as the PID ramps (emits more often than equipmentComposite).
  ...
  async function loadData(equipment: Equipment): Promise<void> {
    ...
    const counts: Counts = { leftRows: 0, rightRows: 0, reserved: refrigStages + auxOutputs, refrigAdded: 0, auxAdded: 0 };
    try {
      ...
      for (let i = 0; i < 10; i += 1) {
        if (equipment.outputConfig[i + 13] !== '-1') refrigStages += 1;
      }
      if (level >= 1) {
        for (let i = 1; i <= 8; i += 1) {
          if (equipment.outputConfig[i + 24] !== '-1' && switch1Use[i] === '5' && switch2Use[i] === '5') auxOutputs += 1;
        }
        if (equipment.outputConfig[4] !== '-1') auxOutputs += 1;
      }
      if (level < 1) { counts.rightRows = 1; counts.leftRows = 1; }

      addEquipment(equipment, 'fan', counts);

      if (potatoMode) {
        if (equipment.outputConfig[3] !== '-1') addEquipment(equipment, 'climacell', counts);
        if (equipment.outputConfig[7] !== '-1') { addEquipment(equipment, 'humid1', counts, true); addEquipment(equipment, 'pump1', counts, true); }
        if (equipment.outputConfig[9] !== '-1') { addEquipment(equipment, 'humid2', counts, true); addEquipment(equipment, 'pump2', counts, true); }
        if (equipment.outputConfig[11] !== '-1') { addEquipment(equipment, 'humid3', counts, true); addEquipment(equipment, 'pump3', counts, true); }
        if (equipment.outputConfig[4] !== '-1') { if (level < 1) addEquipment(equipment, 'heat', counts, true); }
      } else if (onionMode) {
        addEquipment(equipment, 'cure', counts);
        if (equipment.outputConfig[6] !== '-1') addEquipment(equipment, 'burner', counts);
      }

      if (equipment.outputConfig[5] !== '-1') {
        if (equipment.miscData[10] === '0') addEquipment(equipment, 'cavity', counts);
        else addEquipment(equipment, 'pile', counts);
      }
      if (equipment.outputConfig[23] !== '-1') addEquipment(equipment, 'lights1', counts);
      if (equipment.outputConfig[24] !== '-1') addEquipment(equipment, 'lights2', counts);
      if (level >= 1) {
        if (equipment.outputConfig[13] !== '-1' || equipment.pwmConfig[1]?.split(':')[2] !== '-1') addEquipment(equipment, 'refrig', counts);
        addEquipment(equipment, 'door', counts);
      } else {
        if (equipment.outputConfig[13] === '-1' && equipment.pwmConfig[1]?.split(':')[2] !== '-1') addEquipment(equipment, 'refrig', counts);
        if (onionMode && equipment.pwmConfig[3]?.split(':')[2] !== '-1') addEquipment(equipment, 'burner', counts);
        if (equipment.pwmConfig[0]?.split(':')[2] !== '-1') addEquipment(equipment, 'door', counts);
      }

      auxiliaryAdd(equipment, counts);

      if (counts.refrigAdded == 0) { refrigStagesAdd(equipment, counts); counts.refrigAdded = 1; }

      if (potatoMode && equipment.outputConfig[4] !== '-1') {
        if (level >= 1) leftSideAdd(equipment, 'heat', counts);
      }

      auxiliaryAdd(equipment, counts);
    } catch (error) { /* ignore partial-config errors during hydration */ }
    ready = true;
  }
  ...
  async function clear() {
    ...
    await fetch(getHttpUrl('/iot/button'), {
      ...
      body: JSON.stringify({ tag: 'button2', ClearDiag: 'Clear' }),
    });
    wait = false;
  }
  ...
  function auxiliaryAdd(equipment: Equipment, counts: Counts): void {
    if (counts.auxAdded === 0) {
      if (level < 1) {
        for (let i = 1; i <= 8; i += 1) {
          if (equipment.outputConfig[i + 24] !== '-1') addEquipment(equipment, `aux${i}Switch`, counts);
        }
      } else {
        for (let i = 1; i <= 8; i += 1) {
          if (equipment.outputConfig[i + 24] !== '-1') {
            if (switch1Use[i] !== '5' || switch2Use[i] !== '5') leftSideAdd(equipment, `aux${i}Switch`, counts);
          }
        }
      }
      counts.auxAdded = 1;
      ...
      for (let i = 1; i <= 8; i += 1) {
        if (equipment.outputConfig[i + 24] !== '-1') {
          if (switch1Use[i] === '5' && switch2Use[i] === '5') rightSideAdd(equipment, `aux${i}Switch`, counts);
        }
      }
    }
  }
  ...
  function refrigStagesAdd(equipment: Equipment, counts: Counts): void {
    for (let i = 1; i <= 8; i += 1) {
      if (equipment.outputConfig[i + 12] !== '-1') rightSideAdd(equipment, 'refrig' + i, counts);
    }
    if (equipment.outputConfig[21] !== '-1') rightSideAdd(equipment, 'defrost1', counts);
    if (equipment.outputConfig[22] !== '-1') rightSideAdd(equipment, 'defrost2', counts);
  }

  // pwm_doors_pct (SystemStatus field 20) — commanded door PWM %; fills the
  // door row's status column even without DI feedback on the actuators.
  ...
  function rightSideAdd(equipment: Equipment, equipmentName: string, counts: Counts): void {
    ...
    if (eq) { rightRows.push(eq); counts.rightRows += 1; }
  }
  function leftSideAdd(equipment: Equipment, equipmentName: string, counts: Counts): void {
    const eq = getEquipment(equipment, equipmentName, edit, $frontMatterStore?.main as string[], doorPct);
    if (eq) { leftRows.push(eq); counts.leftRows += 1; }
  }

  function humidRows(rows: any[]): any[] { return rows.filter((row) => row.name.startsWith('humid')); }
  function pumpRows(rows: any[]): any[] { return rows.filter((row) => row.name.startsWith('pump')); }
  function refrigRows(rows: any[]): any[] { return rows.filter((row) => row.name.startsWith('refrig') || row.name.startsWith('defrost')); }
</script>
  ...
  <Card class="mt-2 flex flex-col mx-2">
    <div class="flex flex-row mb-2">
      <Table class="{hasRightSide ? 'w-1/2' : ''} mr-2 text-size-xl">
        <Row>
          <Column class="w-1/3">{ $t('global.output') }</Column>
          <Column class="w-1/6">{ $t('global.status') }</Column>
          <Column class="w-1/2">{ $t('global.switch') }</Column>
        </Row>
