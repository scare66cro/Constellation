<script lang="ts">
  ...
  export let data: { name1: string, name2: string };
  ...
  $: lights = {} as { name1: string, name2: string };
  ...
  onMount(async () => {
    try {
      $navigationStore.data = getHttpUrl('/iot/lights');
      $navigationStore.isDirty = () => !isEqual(lights, data);
      lights = cloneDeep(data);
		} catch (error) {
      console.error(error);
		}
		ready = true;
...
<GellertPage {wait} {ready} {title} {level} name="lights">
  <Card class="mt-2 flex flex-col mx-auto container-standard">
    <Table class="text-size-xl">
      ...
      <Row>
        <Column><TextField size="xl" bind:value={lights.name1} {edit} keyboardType={KeyboardTypes.Alpha} validation={validation.lightsBay1Label}/></Column>
      ...
      <Row>
        <Column><TextField size="xl" bind:value={lights.name2} {edit} keyboardType={KeyboardTypes.Alpha} validation={validation.lightsBay2Label}/></Column>
    ...
    <SaveButton {edit} bind:wait={wait} data={lights} bind:original={data} route="lights" bind:validation={validation} autoSave />
  </Card>
</GellertPage>
