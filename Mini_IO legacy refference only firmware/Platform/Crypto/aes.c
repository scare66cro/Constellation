...
void AesCfbEncrypt(Aes* aes, byte* out, const byte* in, word32 len)
{
    unsigned int l=0;
    unsigned int n=0;
    ...
    while(l<len)
    {
        ...
        out[l]=((byte*)aes->reg)[n]^=in[l];
        l++;
        n=(n+1)%16;
    }
}

void AesCfbDecrypt(Aes* aes, byte* out, const byte* in, word32 len)
{
    unsigned int l=0;
    unsigned int n=0;
    unsigned char c;
