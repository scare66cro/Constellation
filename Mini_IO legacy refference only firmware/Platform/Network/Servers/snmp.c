...
static void password_to_key(unsigned char *password, unsigned char *key, unsigned int auth_type);
...
int snmp_process(unsigned char *packet_in, unsigned int length_in, unsigned char *packet_out)
{
    ...
    if ((bytes_used=asn_decode_int(p1, ASN_INT, &version_number))==0) return 0;
    
    if ((version_number==0)||(version_number==1))
    {
    	if (params.snmp.use_v1)
    	{
    		return snmp_process_v1(packet_in, length_in, packet_out);
    	}
    	else
    	{
    		debug_printf("Received V1 SNMP, but V1 is disabled\r\n");
    		return 0;
    	}
    }
    else if (version_number==3)
    {
    	if (params.snmp.use_v3)
    	{
...
// checkes version and hands off to correct processing function, snmp_process_v1 or snmp_process_v3
...
static int snmp_process_v3(unsigned char *packet_in, unsigned int in_length, unsigned char *packet_out)
{
    ...
    else // check for a valid user
    {
        ...
        {
            if (memcmp(params.snmp.users[user_index].username, in_data.v3.msg_username, len)==0) break;
        }
    ...
    if (((params.snmp.users[user_index].auth_type!=AUTH_NONE)&&((in_data.v3.msg_flags&MSG_SEC_AUTH)==0))||
        ((params.snmp.users[user_index].auth_type==AUTH_NONE)&&((in_data.v3.msg_flags&MSG_SEC_AUTH)!=0))) report_error=RPT_ERR_AUTH_FAIL;
    if (((params.snmp.users[user_index].priv_type!=PRIV_NONE)&&((in_data.v3.msg_flags&MSG_SEC_PRIV)==0))||
        ((params.snmp.users[user_index].priv_type==PRIV_NONE)&&((in_data.v3.msg_flags&MSG_SEC_PRIV)!=0))) report_error=RPT_ERR_AUTH_FAIL;
    ...
    if (in_data.v3.msg_flags&MSG_SEC_PRIV) // encrypted
    {
        ...
        p1+=bytes_used;
        // decrypt it
        localize_key(params.snmp.users[user_index].auth_type, params.snmp.users[user_index].priv_key, in_data.v3.lkey, in_data.v3.msg_authoritative_engine_id, in_data.v3.msg_authoritative_engine_id_len);
        decrypt_pdu(params.snmp.users[user_index].priv_type, in_data.v3.lkey, p1, len, in_data.v3.msg_privacy_parameters, in_data.v3.msg_authoritative_engine_boots, in_data.v3.msg_authoritative_engine_time);
    }
    ...
    if ((in_data.v3.request_type!=REPORT_PDU)&&(in_data.v3.request_type!=GET_RESPONSE_PDU))
    {
        ...
        if ((abs((uptime_ms)-in_data.v3.msg_authoritative_engine_time)>=150)||
            ((in_data.v3.msg_authoritative_engine_boots!=params.snmp.boots)))
        {
        ...
        if ((in_data.v3.msg_authoritative_engine_id_len!=params.snmp.engine_id_len)||(memcmp(in_data.v3.msg_authoritative_engine_id, params.snmp.engine_id, params.snmp.engine_id_len)!=0))
        {
        ...
        if (report_error==RPT_ERR_NONE)
        {
        	switch(in_data.v3.request_type)
		    {
		    	...
		    	case GET_NEXT_REQUEST_PDU:
		    		if (params.snmp.users[user_index].user_level==USER_LEVEL_NONE)
		    		{
		    			debug_printf("V3 GET Request for user with User Level None\r\n");
		    			report_error=RPT_ERR_AUTH_FAIL;
		    		}
		    		break;
		    		
		    	case SET_REQUEST_PDU:
		    		if (params.snmp.users[user_index].user_level!=USER_LEVEL_READ_WRITE)
		    		{
		    			debug_printf("V3 SET Request for user without Read/Write access\r\n");
		    			report_error=RPT_ERR_AUTH_FAIL;
		    		}
		    		break;
		    }
    ...
    if (report_error!=RPT_ERR_NONE)
    {
        ...
        memcpy(out_data.v3.msg_authoritative_engine_id, params.snmp.engine_id, params.snmp.engine_id_len);
        out_data.v3.msg_authoritative_engine_id_len=params.snmp.engine_id_len;
        out_data.v3.msg_authoritative_engine_boots=params.snmp.boots;
        ...
        out_data.v3.error_index=0;
        out_data.v3.user=&params.snmp.users[0];
        // no repeaters
    ...
    else if (in_data.v3.request_type==REPORT_PDU)
    {
        ...
        if (cb_struct.wait_report)
        {
            ...
            if (params.snmp.users[cb_struct.user_index].auth_type!=AUTH_NONE)  out_data.v3.msg_flags|=0x01;
            if (params.snmp.users[cb_struct.user_index].priv_type!=PRIV_NONE)  out_data.v3.msg_flags|=0x02;
            out_data.v3.msg_security_model=3;
            ...
            strcpy((char*)out_data.v3.msg_username, (const char*)params.snmp.users[cb_struct.user_index].username);
            out_data.v3.request_type=GET_REQUEST_PDU;
            out_data.v3.request_id=in_data.v3.request_id;
            out_data.v3.error_status=0;
            out_data.v3.error_index=0;
            out_data.v3.user=&params.snmp.users[cb_struct.user_index];
            out_data.v3.var_bindings=snmp_varbindings;
    ...
    else
    {
        ...
        if (out_data.v3.var_bindings_len>0) // build reply packet
        {
            ...
            out_data.v3.msg_max_size=SNMP_MAX_LEN;
            out_data.v3.msg_flags=0;
            if (params.snmp.users[user_index].auth_type!=AUTH_NONE) out_data.v3.msg_flags|=0x01;
            if (params.snmp.users[user_index].priv_type!=PRIV_NONE) out_data.v3.msg_flags|=0x02;
            out_data.v3.msg_security_model=3;
            memcpy(out_data.v3.msg_authoritative_engine_id, params.snmp.engine_id, params.snmp.engine_id_len);
            out_data.v3.msg_authoritative_engine_id_len=params.snmp.engine_id_len;
            out_data.v3.msg_authoritative_engine_boots=in_data.v3.msg_authoritative_engine_boots;
            out_data.v3.msg_authoritative_engine_time=in_data.v3.msg_authoritative_engine_time;
            strcpy((char*)out_data.v3.msg_username, (const char*)params.snmp.users[user_index].username);
            out_data.v3.request_type=GET_RESPONSE_PDU;
            out_data.v3.request_id=in_data.v3.request_id;
            out_data.v3.error_status=0;
            out_data.v3.error_index=0;
            out_data.v3.user=&params.snmp.users[user_index];
            out_data.v3.var_bindings=snmp_varbindings;
...
static int build_snmp_v3(unsigned char *out_buf, _snmp_data *data)
{
    ...
    p1+=asn_encode_octet_string(p1, data->v3.user->username, strlen((const char*)data->v3.user->username));
    ...
    {
        // we fake it here since the encrypted data is stored as an octet_string and the asn_encode_octet_string tryes to actually encode it
...
static int snmp_process_v1(unsigned char *packet_in, unsigned int in_length, unsigned char *packet_out)
{
    ...
    // to indicate a primitive integer of length 1 and value 0 for the SNMP
    // version.  Anything else is wrong
    if ((bytes_used=asn_decode_int(p1, ASN_INT, &in_data.v1.msg_version))==0) return 0;
    ...
    // get PDU packet type
    if ((bytes_used=asn_decode_request_type(p1,&in_data.v1.request_type,&len))==0) return 0;
    p1+=bytes_used;
    ...
    if ((in_data.v1.request_type!=GET_RESPONSE_PDU)&&(in_data.v1.request_type!=GET_REQUEST_PDU)&&(in_data.v1.request_type!=SET_REQUEST_PDU)&&(in_data.v1.request_type!=GET_NEXT_REQUEST_PDU))
    {
        ...
        {
            debug_printf("Unsupported or unknown RequestPDU %x\n", in_data.v1.request_type);
            return 0;
        }
    }
    ...
    switch(in_data.v1.request_type)
    {
    	...
    	case GET_REQUEST_PDU:
    	case GET_NEXT_REQUEST_PDU:
    		if (strcmp((char*)in_data.v1.community, (char*)params.snmp.read_community)!=0)
    		{
    			debug_printf("Invalid Read Community: Got '%s' need '%s'\r\n", in_data.v1.community, params.snmp.read_community);
    			return 0;
    		}
    		break;
    		
    	case SET_REQUEST_PDU:
    		debug_printf("Checking set request PDU\r\n");
    		if (strcmp((char*)in_data.v1.community, (char*)params.snmp.write_community)!=0)
    		{
    			debug_printf("Invalid Write Community: Got '%s' need '%s'\r\n", in_data.v1.community, params.snmp.write_community);
    			return 0;
    		}
    		break;
    		
    	default:
    		debug_printf("Unchecked request type %d\r\n", in_data.v1.request_type);
    		break;
    ...
    // get request id
    if ((bytes_used=asn_decode_int(p1, ASN_INT, &in_data.v1.request_id))==0) return 0;
    p1+=bytes_used;

    if ((in_data.v1.msg_version==1)&&(in_data.v1.request_type==GET_BULK_PDU)) // get other vals
    {
        // get non-repeaters
        if ((bytes_used=asn_decode_int(p1, ASN_INT, &in_data.v1.non_repeaters))==0) return 0;
        p1+=bytes_used;
        
        // get max-repetitions
        if ((bytes_used=asn_decode_int(p1, ASN_INT, &in_data.v1.max_repetitions))==0) return 0;
        p1+=bytes_used;
    }
    else // error number and index not in version 2c getBulkPDU
    {
        // get error number
        if ((bytes_used=asn_decode_int(p1, ASN_INT, &in_data.v1.error_status))==0) return 0;
        p1+=bytes_used;
                
        // get error index
        if ((bytes_used=asn_decode_int(p1, ASN_INT, &in_data.v1.error_index))==0) return 0;
        p1+=bytes_used;
    }
    ...
    if (out_data.v1.var_bindings_len>0)
    {
        ...
        return build_snmp_v1(packet_out, &out_data);
    }
...
int process_varbindings(unsigned char *in_varbindings, int in_varbindings_len, unsigned char *out_varbindings, int msg_version, int request_type, int non_repeaters, int max_repetitions, int *error_status, int *error_index)
{
    ...
    *error_index=0;
    ...
    while((in_varbindings_len-(p1-in_varbindings))>0)
    {
        ...
        // and are comprimised of an OID and a value
        ...
        {
            debug_printf("Can't decode var binding type 0x%x\n", *p1);
            return 0;
        }
        ...
        if (*error_status==snmp_NO_ERROR)
        {
            ...
            else if (request_type==GET_RESPONSE_PDU)
            {
                ...
                if ((cb_struct.wait_reply)&&(cb_struct.request_cb!=NULL))
                {
                    ...
                    {
                        // check community
                        if (strcmp((const char*)cb_struct.community, (const char*)in_data.v1.community)!=0) return 0;
                    }
...
static void calculate_hmac(int auth_type, unsigned char *auth_key, unsigned char *data, unsigned int data_len, unsigned char *to)
{
    ...
    else
    {
       ...
       debug_printf("not supported auth_type: %d\n", auth_type);
    }
...
static void decrypt_pdu(int priv_type, unsigned char *priv_key, unsigned char *data, unsigned int data_len, unsigned char *salt, unsigned int engine_boots, unsigned int engine_time)
{
    ...
    {
        // not supported at the moment
        debug_printf("not supported decryption!\n");
    }
}

// encryptes a v3 PDU message
static void encrypt_pdu(int priv_type, unsigned char *priv_key, unsigned char *data, unsigned int data_len, unsigned char *salt, unsigned int engine_boots, unsigned int engine_time)
{
    int i;
    ...
    {
        // not supported at the moment
        debug_printf("not supported non-DES Decryption!\n");
    }
...
static void localize_key(unsigned int auth_type, unsigned char *key, unsigned char *lkey, unsigned char *engine_id, unsigned int engine_id_len)
{
  // Now localize the key with the engineID and pass through MD5/SHA to procue the final key
    if (auth_type==AUTH_MD5)
    {
...
// calculates the KEY from a PASSWORD, needs to be done everytime a password, engine id, auth_type or priv_type is changed for a user
static void password_to_key(unsigned char *password, unsigned char *key, unsigned int auth_type)
{
    unsigned char *cp;
...
// setup snmp system by registering UDP scocket, and registering some mib's
void snmp_init(void)
{
    struct udp_pcb *pcb;
...
// everytime a username, password, auth_type or priv_type changes and NetParmas should be saved
void snmp_init_keys(void)
{
    int i;
    
    debug_printf("snmp_init_keys\n");
    
    // setup HMAC keys
    for(i=0; i<MAX_SNMPV3_USERS; i++)
    {
        if ((strlen((const char*)params.snmp.users[i].username)>0)&&(strlen((const char*)params.snmp.users[i].auth_key)>0))
        {
            password_to_key(params.snmp.users[i].auth_pass, params.snmp.users[i].auth_key, params.snmp.users[i].auth_type);
            
            if ((params.snmp.users[i].priv_type!=PRIV_NONE)&&(strlen((const char*)params.snmp.users[i].priv_key)>0))
            {
                password_to_key(params.snmp.users[i].priv_pass, params.snmp.users[i].priv_key, params.snmp.users[i].auth_type);
            }
        }
    }
}
