...
static int kfs_internal_read(int fd_index, unsigned long long byte_offset, void *buffer, unsigned int length)
{
	...
	if (byte_offset%SECTOR_SIZE)
	{
		...
		{	
			...
			if (kfs_read_sector(buffered_sectors[fd_index].sector, sector_number, 1)!=KFS_SUCCESS)
			{
				debug_printf("Failed reading disk once, going to try again\r\n");
...
//					log_event(EVENT_NUMBER_DISK_201);
	...
	while(length>0)
	{
		...
		{	
			...
			if (kfs_read_sector(buffered_sectors[fd_index].sector, sector_number, 1)!=KFS_SUCCESS)
			{
				debug_printf("Failed reading disk once, going to try again, tried to read sector %d\r\n", sector_number);
				//extern int extra_debug;
				//extra_debug=1;
				if (kfs_read_sector(buffered_sectors[fd_index].sector, sector_number, 1)!=KFS_SUCCESS)
				{
					debug_printf("Nope...still bad, returning bad disk\r\n");
					return KFS_BADDISK;
				}
				else
				{
...
{
	//debug_printf("kfs_free start: start=%d, read=%d, write=%d, size=%d\r\n", kfs.files[fd_index].start_index, kfs.files[fd_index].read_index, kfs.files[fd_index].write_index, kfs.files[fd_index].file_size);
//	spi_lock(SPI_LOCK_SD, 1);
  spi_lock(1);
	
	if (kfs.files[fd_index].file_size<length)
	{
		kfs.files[fd_index].start_index=0;
		kfs.files[fd_index].read_index=0;
		kfs.files[fd_index].write_index=0;
		kfs.files[fd_index].file_size=0;
	}
	else
	{
		if (kfs.files[fd_index].write_index>kfs.files[fd_index].start_index)
		{
			kfs.files[fd_index].start_index+=length;
			kfs.files[fd_index].file_size-=length;
		}
		else
		{
			if ((kfs.files[fd_index].start_index+length)>=kfs.files[fd_index].allocated_bytes)
			{
				length-=(kfs.files[fd_index].allocated_bytes-kfs.files[fd_index].start_index);
				kfs.files[fd_index].start_index=0;
			}
	...
	//debug_printf("kfs_free stop: start=%d, read=%d, write=%d, size=%d\r\n\r\n", kfs.files[fd_index].start_index, kfs.files[fd_index].read_index, kfs.files[fd_index].write_index, kfs.files[fd_index].file_size);
...
//	  spi_unlock(SPI_LOCK_SD);
    ...
    if (copy1>0)
    {
	    if ((bytes_read=kfs_internal_read(fd_index, read_index, buffer, copy1))!=copy1)
	    {
	    	...
	    	debug_printf("READ: ERROR: copy1 failed: copy1=%d, bytes_read=%d\r\n", copy1, bytes_read);
//	    	spi_unlock(SPI_LOCK_SD);
	      spi_unlock();
	    	return 0;
	    }
	...
	{    
	    ...
	    {
	    	if (bytes_read<0) disk_state=(KFS_RET)bytes_read;
	    	debug_printf("READ: ERROR: copy2 failed: copy2=%d, bytes_read=%d\r\n", copy2, bytes_read);
//	    	spi_unlock(SPI_LOCK_SD);
	      spi_unlock();
	    	return 0;
	    }
...
//    	spi_unlock(SPI_LOCK_SD);
	...
	{    
	    if ((bytes_written=kfs_internal_write(fd_index, write_index, buffer, copy1))!=copy1)
	    {
	    	...
	    	debug_printf("WRITE: ERROR on copy1: copy1=%d, bytes_written=%d\r\n", copy1, bytes_written);
//	    	spi_unlock(SPI_LOCK_SD);
	      spi_unlock();
	    	return 0;
	    }
    ...
    if (copy2>0)
    {
	    ...
	    {
	    	if (bytes_written<0) disk_state=(KFS_RET)bytes_written;
	    	debug_printf("WRITE: ERROR on copy2: copy2=%d, bytes_written=%d\r\n", copy2, bytes_written);
//	    	spi_unlock(SPI_LOCK_SD);
	      spi_unlock();
	    	return 0;
	    }
