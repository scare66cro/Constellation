...
/* 0.A.85 RESTORED: 0.A.82 disabled these to free 288 KiB BSS for OTA's
 * block buffer; 0.A.85 reverted that buffer so we no longer need the
 * memory. AND — per `memories/repo/lp-msram-bloat-audit-may2026.md`,
 * disabling these flags + dropping udp_iperf.c builds clean but
 * SILENTLY BREAKS the LP→bridge UART2 TX path: LP goes silent on
 * UART after boot, bridge `connected:false`, rxFrames frozen at 0.
 * One of the demo init calls is doing something load-bearing for our
 * bridge TX path; we've never bisected which. Until that's done, all
 * four flags must stay enabled and udp_iperf.c must stay in the
 * makefile. The 288 KiB cost is acceptable (current MSRAM headroom
 * is ~127 KiB after restoring this). */
