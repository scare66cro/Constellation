/**
 * nova_crc16.c — CRC-16/CCITT implementation for Constellation protocol
 *
 * Table-driven for speed (~512 bytes of flash for the lookup table).
 * On Cortex-R5F this computes CRC for a 4KB message in ~20µs.
 */
...
uint16_t nova_crc16(const uint8_t *data, size_t len)
{
    uint16_t crc = 0xFFFF;
    while (len--) {
        crc = nova_crc16_update(crc, *data++);
    }
    return crc;
}
