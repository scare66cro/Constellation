/* ============================================================================
 * nova_states.c — Nova-native port of legacy Application/States.c
 * ----------------------------------------------------------------------------
 *
 * MIGRATION STATUS (Phase 15 of legacy → Nova-native):
 *
 *   States.c sits at the centre of the equipment engine. Its job:
 *     - Decide CurrentMode / SystemState / CureState every loop based on
 *       the door switch, cure switch, plenum/outside temps, start temp,
 *       ramp rate, refrig output, and the system schedule.
 *     - Dispatch to the appropriate Mode* worker (ModeAirCure /
 *       ModeBurnerCure / ModeCooling / ModeFanManual / ModeFanOff /
 *       ModeHeating / ModeRecirc / ModeRefrig / ModeShutdown /
 *       ModeStandby) — those workers were ported in Phase 11.
 *
 *   This is a verbatim port of the legacy logic with only the file
 *   header replaced. All state-machine bodies — CheckCureSwitchStatus,
 *   CheckDoorStatus, CheckOutsideTempSensorStatus, CheckPlenumSensor-
 *   Status, CheckStartSwitchStatus, CheckSystemStatus, RefrigRunTimer,
 *   SetDehumidification, SetLightStatus, SetRamp, SetStartTemp,
 *   SetStateCooling, SetStateCure, SetStateDefrost, SetStateHeating,
 *   SetSystemState, SetMode, RunClockMode, FanRunTimer,
 *   CalculatedHumidity — are preserved bit-for-bit so production
 *   equipment behaviour matches the AS2 baseline that was field-validated.
 *
 *   Globals owned here (declared extern in States.h):
 *     RAMP_RATE Ramp; char CurrentMode; char SystemState; char CureState;
 *
