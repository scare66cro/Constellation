...
> **PARTIAL 2026-06-01 (0.A.219).** Architecture landed: proto, lp_settings,
> `nova_vfd_client.{h,c}`, main.c handlers + emitter, bridge senders.
> **NOT YET DONE:** bridge `vfdClient.ts` rewrite to consume
> `VfdRegBank` + dispatch actions via `sendVfdRawWrite`; bench
> validation (gated on rs485Responder running on Pi5 bound to all
> interfaces).

**Architecture decision (2026-06-01):** Option B — **Nova owns Modbus
transport, bridge keeps the manufacturer-aware decode.** The original
plan sketched "Nova encodes `VfdStatus` directly", but porting
`vfdRegisterMaps.ts` + the ABB/Phase-Tech profile heuristics + the
action→CW value mapping to C is a large surface for no airgap benefit.
Instead Nova polls register windows and emits raw `VfdRegBank` banks
(mirror of `OrbitSensorBank`); the bridge subscribes, decodes per
manufacturer profile, and emits `VfdStatus` onto the WS as today. The
manufacturer table stays in TypeScript where new drives drop in
without a Nova reflash.

**Wire contracts (envelope.proto tags 127-129):**

- **`VfdConfig` (Bridge → Nova, tag 127)** — `{ vfd_host_ipv4, vfd_port,
  max_scan_unit_id, poll_interval_ms }`. Bridge sends at startup + on
  operator config change. Nova caches in `LpSettings.vfd`.
- **`VfdRegBank` (Nova → Bridge, tag 128)** — `{ unit_id, hr_base,
  values[], seq, online }`. One bank per (unit, group) per cycle. Six
  groups per drive: process 0-3, live 100-108, fault 400, limits
  2001-2002, ramp 2202-2203, nameplate 9901-9905.
- **`VfdRawWrite` (Bridge → Nova, tag 129)** — `{ unit_id, addr,
  values[] }`. Nova picks FC06 for 1 value, FC16 for ≥2.

**Landed in 0.A.219 (this commit):**

1. `proto/agristar/vfd.proto` + `envelope.proto` — three new messages.
2. `Nova_Firmware/lp_am2434/lp_settings.{h,c}` — `LpVfdConfig` field +
   `LpSettings_SetVfdConfig` / `LpSettings_GetVfdConfig`. Not yet
   OSPI-persisted (bridge replays on every connect).
3. `Nova_Firmware/lp_am2434/nova_vfd_client.{h,c}` — new file, ~430
   lines. Single FreeRTOS task (8 KB stack, MSRAM-budgeted to 4
   drives), polls all 6 groups per discovered unit at
   `poll_interval_ms`, exposes `NovaVfdClient_DriveCount` +
   `NovaVfdClient_GetSnapshot` for the emitter, dedicated short-lived
   socket for `NovaVfdClient_WriteRegisters` so the polling task's
   socket isn't disturbed.
4. `Nova_Firmware/lp_am2434/main.c` — `VfdConfig` handler at field
   127, `VfdRawWrite` handler at field 129, `build_vfd_reg_bank_envelope`
   builder, cycling-poller emit in `bridge_uart_task` (one drive per
   100 ms tick → 6 banks per cycle → ~800 ms full refresh per drive
   with 4 drives).
5. `Nova_Firmware/lp_am2434/lwip_smoke.c` — `NovaVfdClient_Init()`
   call alongside `OrbitClient_Init()`.
6. `Nova_Firmware/lp_am2434/ti-arm-clang/makefile` — adds
   `nova_vfd_client.c` to FILES.
7. `constellation-ui/server/src/novaSerialBridge.ts` — `MSG_VFD_*`
   constants, `sendVfdConfig`, `sendVfdRawWrite`.

**Pending for the next session:**

1. **Bridge `index.ts`** — call `novaBridge.sendVfdConfig(VFD_HOST,
   VFD_PORT, VFD_MAX_SCAN, 1000)` after the protocol-1 handshake +
   on any config change. The current `VFDClient` instantiation stays
   in parallel until step 2 lands.
2. **Bridge `novaDataStore.ts`** — `decodeVfdRegBank` at envelope tag
   128, keyed `Map<unit_id, Map<hr_base, VfdRegBank>>`. Mirror of
   `decodeOrbitSensorBank`.
3. **Bridge `vfdClient.ts`** — rewrite to subscribe to the VfdRegBank
   stream + dispatch actions via `sendVfdRawWrite`. Keep the existing
   `getDrives()` / `sendAction()` API so `apiRoutes.ts` doesn't need
   to change. Delete the `modbus-serial` import.
4. **Bridge `index.ts`** — drop the `new VFDClient({ host, port })`
   instantiation; the rewritten module exposes a singleton that
   takes no constructor args.
5. **Bench validation** — start `rs485Responder` on Pi5 bound to
   `0.0.0.0:5020` so Nova at `10.47.27.1` can reach it via Pi5's
   bench IP `10.47.27.108:5020`. Verify `/iot/fans` reflects polled
   data + start/stop control.
