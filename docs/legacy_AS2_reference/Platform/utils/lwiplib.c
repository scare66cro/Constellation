...
//
//*****************************************************************************
static struct ip_addr g_ui32IPAddr;
...
//
//*****************************************************************************
static struct ip_addr g_ui32NetMask;
...
//
//*****************************************************************************
static struct ip_addr g_ui32GWAddr;
...
static void lwIPInterruptTask(void *pvArg)
{
    while(1)
    {
        ...
        MAP_EMACIntEnable(EMAC0_BASE, (EMAC_INT_RECEIVE | EMAC_INT_TRANSMIT | EMAC_INT_TX_STOPPED | EMAC_INT_RX_NO_BUFFER | EMAC_INT_RX_STOPPED | EMAC_INT_PHY));
    }
}
#endif
...
static void lwIPLinkDetect(void)
{
    ...
    unsigned int bmsr = MAP_EMACPHYRead(EMAC0_BASE, 0, EPHY_BMSR);
    bHaveLink =  bmsr & EPHY_BMSR_LINKSTAT;

    // Return without doing anything else if the link state hasn't changed.
    if(bHaveLink == g_bLinkActive) return;

    debug_printf("CHANGE: bmsr: %X\r\n", bmsr);
...
#if LWIP_DHCP
        if(g_ui32IPMode == IPADDR_USE_DHCP) dhcp_start(&g_sNetIF);
#endif

        // Start AutoIP, if enabled and DHCP is not.
#if LWIP_AUTOIP
        if(g_ui32IPMode == IPADDR_USE_AUTOIP) autoip_start(&g_sNetIF);
#endif
    }
    else
    {
...
#if LWIP_DHCP
        if(g_ui32IPMode == IPADDR_USE_DHCP) dhcp_stop(&g_sNetIF);
#endif

        // Stop AutoIP, if enabled and DHCP is not.
#if LWIP_AUTOIP
        if(g_ui32IPMode == IPADDR_USE_AUTOIP) autoip_stop(&g_sNetIF);
#endif
    }
}
#endif
...
static void lwIPPrivateInit(void *pvArg)
{
    ...
    // If using a RTOS, create a queue (to be used as a semaphore) to signal the Ethernet interrupt task from the Ethernet interrupt handler.
#if !NO_SYS
#if RTOS_FREERTOS
...
#if !NO_SYS
#if RTOS_FREERTOS
    if (xTaskCreate(lwIPInterruptTask, (signed portCHAR *)"eth_int", STACKSIZE_LWIPINTTASK, 0, tskIDLE_PRIORITY + 1, 0) != pdTRUE)
    {
    	debug_printf("\r\n\r\n!!!!!Failed to start ether_int thread!!!!\r\n");
    }
#endif
#endif
    ...
    {
        ip_addr = g_ui32IPAddr;
        net_mask = g_ui32NetMask;
        gw_addr = g_ui32GWAddr;
    }
    else
    {
        ip_addr.addr = 0;
    ...
    // Create, configure and add the Ethernet controller interface with default settings.
    // ip_input should be used to send packets directly to the stack when not using a RTOS
    // tcpip_input should be used to send packets to the TCP/IP thread's queue when using a RTOS.
#if NO_SYS
...
void lwIPEthernetIntHandler(void)
{
    ...
    if(ui32Status) MAP_EMACIntClear(EMAC0_BASE, ui32Status);
    ...
    if(ui32Status & EMAC_INT_TIMESTAMP)
    {
        ...
        if(g_pfnTimerHandler) g_pfnTimerHandler(EMAC0_BASE, ui32TimerStatus);
    }

    // The handling of the interrupt is different based on the use of a RTOS.
#if NO_SYS
    // No RTOS is being used.  If a transmit/receive interrupt was active, run the low-level interrupt handler.
    if(ui32Status) tivaif_interrupt(&g_sNetIF, ui32Status);
    ...
    //
    // Disable the Ethernet interrupts.  Since the interrupts have not been handled, they are not asserted.
    // Once they are handled by the Ethernet interrupt task, it will re-enable the interrupts.
    MAP_EMACIntDisable(EMAC0_BASE, (EMAC_INT_RECEIVE | EMAC_INT_TRANSMIT | EMAC_INT_TX_STOPPED | EMAC_INT_RX_NO_BUFFER | EMAC_INT_RX_STOPPED | EMAC_INT_PHY));
...
{
#if LWIP_AUTOIP || LWIP_DHCP
    if(g_bLinkActive) return((uint32_t)g_sNetIF.ip_addr.addr);
...
static void lwIPPrivateNetworkConfigChange(void *pvArg)
{
    ...
    switch(g_ui32IPMode)
    {
        ...
        {
            // Set the new address parameters.  This will change the address configuration in lwIP, and if necessary, will reset any links that are active.  This is valid for all three modes.
            netif_set_addr(&g_sNetIF, &ip_addr, &net_mask, &gw_addr);

            // If we are going to DHCP mode, then start the DHCP server now.
#if LWIP_DHCP
            if((ui32IPMode == IPADDR_USE_DHCP) && g_bLinkActive) dhcp_start(&g_sNetIF);
#endif

            // If we are going to AutoIP mode, then start the AutoIP process now.
#if LWIP_AUTOIP
            if((ui32IPMode == IPADDR_USE_AUTOIP) && g_bLinkActive) autoip_start(&g_sNetIF);
#endif
        ...
        {
            // If we are going to static IP addressing, then disable DHCP and force the new static IP address.
            if(ui32IPMode == IPADDR_USE_STATIC)
            {
                dhcp_stop(&g_sNetIF);
                netif_set_addr(&g_sNetIF, &ip_addr, &net_mask, &gw_addr);
            ...
            // If we are going to AUTO IP addressing, then disable DHCP, set the default addresses, and start AutoIP.
#if LWIP_AUTOIP
        ...
        {
            // If we are going to static IP addressing, then disable AutoIP and force the new static IP address.
            if(ui32IPMode == IPADDR_USE_STATIC)
            {
            ...
            // If we are going to DHCP addressing, then disable AutoIP, set the default addresses, and start dhcp.
#if LWIP_DHCP
...
//
//*****************************************************************************
void lwIPNetworkConfigChange(struct ip_addr ui32IPAddr, struct ip_addr ui32NetMask, struct ip_addr ui32GWAddr, uint32_t ui32IPMode)
{
    //
    // Check the parameters.
    //
    ...
    // Save the network configuration for later use by the private network configuration change.
    g_ui32IPAddr = ui32IPAddr;
    g_ui32NetMask = ui32NetMask;
    g_ui32GWAddr = ui32GWAddr;

    // Complete the network configuration change.  The remainder is done immediately if not using a RTOS and it is deferred to the TCP/IP thread's context if using a RTOS.
#if NO_SYS
