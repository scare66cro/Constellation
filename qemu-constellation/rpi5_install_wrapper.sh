...
SSHPASS="$PW" sshpass -e ssh $SSH_OPTS "$HOST" 'cat > /tmp/lighttpd_clean.conf' < /mnt/f/Constellation/qemu-constellation/rpi5_lighttpd_clean.conf

# Stage the install script
SSHPASS="$PW" sshpass -e ssh $SSH_OPTS "$HOST" 'cat > /tmp/install.sh' < /mnt/f/Constellation/qemu-constellation/rpi5_install_clean_lighttpd.sh
